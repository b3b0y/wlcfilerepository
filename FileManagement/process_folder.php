		...
		$State = $row['UserName'] ." is Created a New Folder ".$_POST['file'];
			...
			(substr($folder, -1, 1) != "/" && !empty($folder) ? $folder .= "/" : $folder);
				...
				else
				{
					...
					$new_path = $dir_to_browse_base."/".$file_name;
				}
			...
			if(!is_dir($new_path))
			{
				...
				if(empty($_POST['folder']))
				{
					...
					if(mysql_num_rows($result) > 0 )
					{
						...
						mysql_query("INSERT INTO fr_path(pathName,Folder_Name,Parent_F) VALUES('".$new_path."','".$file_name."','".$row['pathID']."')");

						$result2 = mysql_query("SELECT * FROM fr_path  WHERE Folder_Name = '".$file_name."'");
						$row2 = mysql_fetch_array($result2);

						mysql_query("INSERT INTO fr_folder_owner(Fid,uid) VALUES('".$row2['pathID']."','".$_SESSION['uid']."')");
					}
					else
					{
						...
						if(mysql_num_rows($result) > 0 )
						{
							...
							mysql_query("INSERT INTO fr_ins_subject(uid,Subject,SubPath,Folder_Owner,Parent_F,Date_Created,Time_Created) VALUES('".$_SESSION['uid']."','".$file_name."','".$new_path."','".$_SESSION['uid']."','".$row['ID']."','".$date ."','".$time."')");
