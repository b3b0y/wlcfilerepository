		...
		if($listing_mode == 0)
		{
			...
			foreach($content as $key => $val)
			{
				if(!in_array($val, $exclude))
				{
					...
					{
						/*
						$result2 = mysql_query("SELECT * FROM  fr_user  WHERE UserID = '".$_SESSION['uid']."'");
						$row2 = mysql_fetch_array($result2);
						if($row2['UserLvl'] >= 3 )
						{
							$folders['name'][] = $val;
							$folders['date'][] = date("d F Y", filectime($item_path));
							$folders['link'][] = (empty($url_folder[count($url_folder)-1])) ? $val : $url_folder[count($url_folder)-1]."/".$val;
						}
						else
						{		
					...
					if(!in_array($item, $exclude))
					{
						...
						if(is_dir($item_path))
						{
							if($_SESSION['UserLvl'] >= 3)
							{
								$folders['name'][] = $item;
								$folders['date'][] = date("d F Y", filectime($item_path));
								$folders['link'][] = (empty($url_folder)) ? $item : $url_folder."/".$item;
							...
							else
							{
								...
								while ($row = mysql_fetch_array($result)) 
								{
									if($row['Folder_Name'] == $item && $row['pathName'] == $item_path)
									{
										$folders['name'][] = $item;
										$folders['date'][] = date("d F Y", filectime($item_path));
										$folders['link'][] = (empty($url_folder)) ? $item : $url_folder."/".$item;
									...
									$result2 = mysql_query("SELECT * FROM  fr_stud_subject  WHERE studID = '".$_SESSION['uid']."'");
									if(mysql_num_rows($result2) > 0)
									{
										while ($row2 = mysql_fetch_array($result2)) {
											if($row2['Folder_Name'] == $item)
											{
												...
												if(mysql_num_rows($result2) > 0)
												{
													...
													$folders['name'][] = basename($row3['SubPath']);
													$folders['date'][] = date("d F Y", filectime($item_path));
													$folders['link'][] = (empty($url_folder)) ? $item : $url_folder."/".$item;
												}
											}
									...
									$result3 = mysql_query("SELECT * FROM   fr_share_folder  WHERE studID = '".$_SESSION['uid']."'");
									if(mysql_num_rows($result3) > 0)
									{
										while ($row3 = mysql_fetch_array($result3)) 
										{
											if($row3['Folder_Name'] == $item)
											{
												$folders['name'][] = $row3['Sub_Name'];
												$folders['date'][] = date("d F Y", filectime($item_path));
												$folders['link'][] = (empty($url_folder)) ? $item : $url_folder."/".$item;
											}
										}
						...
						else
						{
							...
							if(!($item == $this_file_name && $this_file_size == $file_size))//Exclude the main index file specifically
							{
								$file_ext = strrchr($item, ".");
								...
								if(!in_array($file_ext, $exclude_ext))
								{
									$files['name'][] = $item;
									$files['size'][] = $file_size;
									$files['link'][] = $path.rawurlencode($item);
									$files['date'][] = date("d F Y", filectime($item_path));
									if($images_detected == '')
										$images_detected = (in_array(strtolower($file_ext), array('.jpeg', '.jpg', '.png', '.gif'))) ? 1 : 0;
									if($media_detected == '')
...
{
	return '
	<table width="100%" cellpadding="5" cellspacing="0" style="font-size:14px;color:gray;">
		<tr>
...
function icons($files, $view_mode)
{
	...
	$videos_qt = array('.mov', '.qt', '.mp4', '.swf');
...
function set_local_text($language_id)
{
	...
	*/
	...
	'switch_to_list' => array('<img src="Images/Details.gif" width="40px">', 'Passez à vue de liste','Umschalten auf Listenansicht','Cambiar a vista de lista'),
	
	'switch_to_thumbnail' => array('<img src="Images/Large icon.gif" width="30px">','Switch to vignette','Umschalten auf Symboldarstellung ','Cambiar a vista de miniaturas'),
	
	'switch_to_listText' => array('Details', 'Passez à vue de liste','Umschalten auf Listenansicht','Cambiar a vista de lista'),

	'switch_to_thumbnailText' => array('Thumbnail','Switch to vignette','Umschalten auf Symboldarstellung ','Cambiar a vista de miniaturas'),
